...
```csharp
// 하이브리드 설계의 핵심 예시: 메인 스레드 안전성 보장
public async Task<MapData> LoadAndProcessMapData()
{
    // 1. 순수 연산은 백그라운드 격리 (멀티 코어 활용)
    byte[] fileData = await Task.Run(() => File.ReadAllBytes("map.bin"));
    
    // 2. 메인 스레드로 돌아와 유니티 오브젝트 안전하게 생성

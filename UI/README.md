...
1. 효율적인 데이터 이동 전략 (Move & Clear)<br>
...
3. 하이브리드 이벤트 시스템 (Button + IPointer)<br>
...
DragAndDrop: 드래그 앤 드롭의 전역 상태와 데이터 전송 함수를 총괄하는 매니저.<br>
DragIcon: 드래그 중인 아이템의 시각적 피드백을 담당하는 오버레이 아이콘.<br>
DropSlot: IPointer 인터페이스를 통해 드롭 이벤트를 감지하고 데이터를 수신하는 컴포넌트.

InventoryBase<T> (Abstract): 제네릭을 사용하여 다양한 데이터 타입을 수용할 수 있는 인벤토리 최상위 클래스.<br>
Inventory: InventoryBase를 상속받아 실제 게임 로직에 연결된 실체화 클래스.<br>
SlotBase<T> (Abstract): 슬롯의 핵심 기능을 정의한 제네릭 베이스 클래스.<br>
